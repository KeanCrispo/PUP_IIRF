...
const RASPBERRY_PI_STREAM_URL = "http://10.14.245.203:8000/stream.mjpg"; 
...
const DashboardPage: React.FC = () => {
  ...
  const lastGreetingRef = useRef<number>(0); // timestamp of last played greeting (ms)

  // --- new: audio context & preloaded audio elements + user gesture unlock ---
  const audioContextRef = useRef<AudioContext | null>(null);
  const audioAllowedRef = useRef(false);
  const audioElsRef = useRef<Record<string, HTMLAudioElement>>({
    morning: new Audio('/audio/Good_Morning.mp3'),
    afternoon: new Audio('/audio/Good_Afternoon.mp3'),
    evening: new Audio('/audio/Good_Evening.mp3'),
  });
  ...
  useEffect(() => {
    const enableAudio = async () => {
      try {
        ...
        // if anything fails, we still attempt to play via Audio API later
  ...
  // Play greeting audio (enforce 1 minute interval)
  const playGreetingIfAllowed = async () => {
    const now = Date.now();
    const ONE_MIN = 60_000;
    if (now - lastGreetingRef.current < ONE_MIN) {
      return;
    }
    ...
    const audioEl = audioElsRef.current[key];

    // Try to ensure audio context is resumed if user already interacted
    try {
    ...
    // Attempt to play. If blocked, try a short resume via context then play again.
    try {
      await audioEl.play();
      lastGreetingRef.current = now;
      return;
    } catch (err) {
      console.warn('Initial audio.play() blocked, attempting resume:', err);
    }

    // As fallback attempt to resume audio context and play again
    try {
      if (!audioAllowedRef.current && audioContextRef.current) {
        await audioContextRef.current.resume();
        audioAllowedRef.current = audioContextRef.current.state === 'running';
      }
    } catch {}
    ...
    try {
      ...
      console.warn('Greeting audio play failed after resume attempt:', err);
    }
  };
        ...
        img.onload = async () => {
          ...
          if (detections.length > 0 && !isFaceRegistered(detections)) {
            ...
            try {
              ...
              // play greeting when an image/detection is uploaded
              try { await playGreetingIfAllowed(); } catch (e) { /* ignore */ }
            } catch (err) {
              console.warn('Failed to insert raspi detection:', err);
            }
          }
        };
      }
            ...
            if (memberDescriptors.length > 0) {
              ...
              for (const det of detectionsWithDescriptors) {
                ...
                if (bestMatch.label !== 'unknown') {
                  try {
                    ...
                    console.warn('Failed to insert known detection:', err);
                  }
                } else {
                  try {
                    await supabase.from("detections").insert([{
                    ...
                    console.warn('Failed to insert unknown detection:', err);
                  }
                }
              }
            } else {
              ...
              for (const det of detectionsWithDescriptors) {
                ...
                try {
                  ...
                  console.warn('Failed to insert detection (no descriptors):', err);
                }
              }
            }
          }
  ...
  // Subscribe to realtime INSERTs on detections and play greeting when image present
  useEffect(() => {
    let channel: any = null;
    let legacySub: any = null;

    // Supabase v2 realtime (channel) API
        ...
        channel = (supabase as any)
          ...
          .on(
            ...
            (payload: any) => {
              ...
              if (inserted && inserted.image) {
                playGreetingIfAllowed();
              }
            }
          )
          .subscribe();
      } catch (err) {
        console.warn('Supabase channel subscribe failed, falling back to legacy API', err);
      }
    }

    // Fallback for older Supabase clients that support .from(...).on(...)
    if (!channel) {
      try {
        // @ts-ignore - older client runtime may expose .on
